...
class WeightedGraph[V](adjList:Map[V, List[WeightedEdge[V]]]) extends Graph[V]{
  ...
  def findExactTrips(source: V, destination: V, stops: Int): Int = {
    ...
    else neighbours(source).foldLeft(0)((routes, neighbour) => (routes + findExactTrips(neighbour, destination, stops - 1)))
  }
  ...
  def findMaxTotalTrips(source: V, destination: V, maxStops: Int): Int = {
    if(maxStops <= 0)0
      ...
      if(source == destination) 1 + neighbours(source).foldLeft(0)((routes, neighbour) => (routes + findMaxTotalTrips(neighbour, destination,  maxStops - 1)))
      else neighbours(source).foldLeft(0)((routes, neighbour) => (routes + findMaxTotalTrips(neighbour, destination,  maxStops - 1)))
    }
  }

  def findTripsInRange(source: V, destination: V, range: Int): Int = {
      if(range < 0) 0
      else {
        val total = neighboursWithWeight(source).foldLeft(0)((routes, neighbour) => (routes + findTripsInRange(neighbour.destination, destination,  range - neighbour.weight)))
